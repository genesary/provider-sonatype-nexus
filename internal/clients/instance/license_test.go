...
func newTestLicenseClient(t *testing.T, serverURL string) instanceclient.LicenseClient {
	t.Helper()

	return instanceclient.NewLicenseClient(nexus.Credentials{
		URL:      serverURL,
		Username: "admin",
		Password: "password",
		Insecure: true,
...
// isErrNoLicense checks whether err wraps instanceclient.ErrNoLicense.
func isErrNoLicense(err error) bool {
	return strings.Contains(err.Error(), instanceclient.ErrNoLicense.Error())
}

// newMinimalLicenseCR returns an empty License CR for testing.
func newMinimalLicenseCR() *instancev1alpha1.License {
	return &instancev1alpha1.License{}
}
...
func TestGetLicense_Success(t *testing.T) {
	...
	want := &instanceclient.LicenseInfo{
		Fingerprint:    "abc123",
...
func TestHashLicense(t *testing.T) {
	...
	hash := instanceclient.HashLicense(data)

	if len(hash) != 64 {
		t.Errorf("HashLicense() length = %d, want 64", len(hash))
	}

	hash2 := instanceclient.HashLicense(data)
	if hash != hash2 {
		t.Error("HashLicense() is not deterministic")
	}

	hashOther := instanceclient.HashLicense([]byte("other-data"))
	if hash == hashOther {
...
func TestHashLicense_Empty(t *testing.T) {
	...
	hash := instanceclient.HashLicense([]byte{})
	if len(hash) != 64 {
...
func TestGenerateLicenseObservation_NilInfo(t *testing.T) {
	...
	obs := instanceclient.GenerateLicenseObservation(nil, prevHash)
...
func TestGenerateLicenseObservation_WithInfo(t *testing.T) {
	...
	info := &instanceclient.LicenseInfo{
		Fingerprint:    "fp-abc",
	...
	hash := "installed-hash"
	obs := instanceclient.GenerateLicenseObservation(info, hash)
...
func TestIsLicenseUpToDate_NoFingerprint(t *testing.T) {
	...
	cr.Status.AtProvider.InstalledHash = instanceclient.HashLicense([]byte("data"))
	cr.Status.AtProvider.Fingerprint = ""

	if instanceclient.IsLicenseUpToDate(cr, cr.Status.AtProvider.InstalledHash) {
...
func TestIsLicenseUpToDate_HashMismatch(t *testing.T) {
	...
	cr.Status.AtProvider.InstalledHash = instanceclient.HashLicense([]byte("old-data"))

	if instanceclient.IsLicenseUpToDate(cr, instanceclient.HashLicense([]byte("new-data"))) {
...
func TestIsLicenseUpToDate_UpToDate(t *testing.T) {
	...
	if !instanceclient.IsLicenseUpToDate(cr, hash) {
