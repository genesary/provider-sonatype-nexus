...
// newTestUserTokenConfig returns a minimal UserTokenConfiguration for tests.
func newTestUserTokenConfig(enabled bool) *iamv1alpha1.UserTokenConfiguration {
	return &iamv1alpha1.UserTokenConfiguration{
		ObjectMeta: metav1.ObjectMeta{Name: "singleton"},
		Spec: iamv1alpha1.UserTokenConfigurationSpec{
			ForProvider: iamv1alpha1.UserTokenConfigurationParameters{
				Enabled: enabled,
			},
		},
	}
...
func TestObserve(t *testing.T) {
	...
	tests := []struct {
		...
		mockSetup    func(*iammocks.MockUserTokenConfigurationClient)
		wantExists   bool
		wantUpToDate bool
		wantErr      bool
	}{
		{
			...
			cr: func() *iamv1alpha1.UserTokenConfiguration {
		...
		{
			name: "ExistsButProtectContentDiffers",
			cr: func() *iamv1alpha1.UserTokenConfiguration {
				cr := newTestUserTokenConfig(true)
				cr.Spec.ForProvider.ProtectContent = &protect
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mc := iammocks.NewMockUserTokenConfigurationClient()
			if tt.mockSetup != nil {
				tt.mockSetup(mc)
			}
...
func TestObserve_WrongType(t *testing.T) {
	...
	e := &external{client: iammocks.NewMockUserTokenConfigurationClient()}
...
func TestCreate(t *testing.T) {
	...
	tests := []struct {
		...
		mockSetup func(*iammocks.MockUserTokenConfigurationClient)
		wantErr   bool
		validate  func(*testing.T, *iammocks.MockUserTokenConfigurationClient)
	}{
		{
			name: "CreateSuccess",
			cr:   newTestUserTokenConfig(true),
...
// TestDelete tests that Delete is a no-op for UserTokenConfiguration.
...
func TestConnect_WrongType(t *testing.T) {
	...
	if err.Error() != errNotUserTokenConfig {
		t.Errorf("Connect() error = %q, want %q", err.Error(), errNotUserTokenConfig)
	}
}
