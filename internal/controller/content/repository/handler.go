...
type FormatHandler interface {
	...
	Observe(ctx context.Context, client *pkgrepository.RepositoryService, name string, repoType string, cr *repositoryv1alpha1.Repository) (exists bool, upToDate bool)

	// Create creates a new repository in Nexus.
	Create(ctx context.Context, client *pkgrepository.RepositoryService, cr *repositoryv1alpha1.Repository, repoType string) error

	// Update updates an existing repository in Nexus.
	Update(ctx context.Context, client *pkgrepository.RepositoryService, name string, cr *repositoryv1alpha1.Repository, repoType string) error

	// Delete deletes a repository from Nexus.
	Delete(ctx context.Context, client *pkgrepository.RepositoryService, name string, repoType string) error
